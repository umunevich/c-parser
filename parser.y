%{
    ...
    void print_tree_json(struct node* tree, FILE *fp);
...
int main() {
	...
	FILE *fp = fopen("/home/umunevich/University/3/System_programming/C_Parser/tree.json", "w");

    if (fp) {
        print_tree_json(head, fp);
        fclose(fp);
        printf("\nParse tree saved to tree.json\n");
    } else {
        printf("Error opening file for JSON output.\n");
    }
...
void print_tree_json(struct node* tree, FILE *fp) {
    ...
    fprintf(fp, "{ \"name\": \"%s\"", tree->token);

    if (tree->left || tree->right) {
        fprintf(fp, ", \"children\": [");
        if (tree->left) print_tree_json(tree->left, fp);
        if (tree->left && tree->right) fprintf(fp, ", ");
        if (tree->right) print_tree_json(tree->right, fp);
        fprintf(fp, "]");
    }
